...
const MOBILE_IMAGES = [mv1, mv2, mv3, mv4];

const MOBILE_PRODUCTS = [
  PRODUCTS[3], // slide 0 → mv1 → Kufrida
  PRODUCTS[0], // slide 1 → mv2 → Hemp Pain Spray
  PRODUCTS[1], // slide 2 → mv3 → OvuCare
  PRODUCTS[2], // slide 3 → mv4 → Vajra-X
];
...
export default function NithyaHome() {
  ...
  const p         = isMobile ? MOBILE_PRODUCTS[active] : PRODUCTS[active];
  const prevProd  = prevActive !== null
                      ? (isMobile ? MOBILE_PRODUCTS[prevActive] : PRODUCTS[prevActive])
                      : null;
  const activeImg = isMobile ? MOBILE_IMAGES[active] : p.img;
  const exitingImg = prevActive !== null
                       ? (isMobile ? MOBILE_IMAGES[prevActive] : prevProd?.img)
                       : null;
  ...
  return (
    <>
      ...
      <footer style={{ background: "#2D5344", color: "#FAF7F1", padding: "32px 0", textAlign: "center", fontFamily: "Lato, sans-serif" }}>
        <p style={{ opacity: 0.8, fontSize: "0.85rem", letterSpacing: "0.1em" }}>© 2026 NITHYA AYURVEDA — Pune, MH</p>
      </footer>
